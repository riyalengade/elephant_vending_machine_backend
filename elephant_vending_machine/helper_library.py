...
INTERTRIAL_TIME = ''#Time interval in between trials
...
def display_stimuli(screen_number, stimuli, correct_stimuli):
    ...
    correct_stimuli (boolean): flag to indicate if this stimuli is the correct one
...
def randomize_stimuli(screen1, screen2):
    ...
    """
    random_num = random.randint(1, 2) #if random_num == 1, stimuli on screens stay the same
    if random_num == 2: #else, switch stimuli on screen
        temp_screen = Screen()
...
def set_intertrial_time(time):
    ...
    response = ''
    if time >= 1:
        global INTERTRIAL_TIME
        INTERTRIAL_TIME = time
        response = 'set inter-trial time to ' + str(INTERTRIAL_TIME)
    else:
